...
//          
            // if (@is_readable($this->basePath."/".$dir)&&file_exists($this->basePath."/".$dir)&& is_dir($this->basePath."/".$dir)) {
                // $tmp_arr[]=$dir;
            // }
//          
        // }
...
class TextCollector_Writer_RailsYaml implements TextCollector_Writer {
    ...
    public function getYaml($entities, $locale,$existing=null) {
        ...
        //Merge with existing translated fields
        $newtrans=array($locale => $entitiesNested);
        if ($existing!=null) {
            foreach ($newtrans as $loc => $locvalue) {
                foreach ($locvalue as $kcontext => $vcontext) {
                    foreach ($vcontext as $klabel => $label) {
        ...
        return $yamlHandler->dump($newtrans, 99);
    }
}
